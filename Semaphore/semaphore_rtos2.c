...
void orangeTask(void *argument)
{
  ...
  {
		orangeTaskProfiler++;
		osSemaphoreAcquire(uart_SemHandle, osWaitForever);
		send_string("this is orange");
...
void blueTask(void *argument)
{
  ...
  for(;;)
  {
		blueTaskProfiler++;
		//osSemaphoreAcquire(uart_SemHandle, osWaitForever);
		//send_string("this is Blue"); //it will not print as semaphore not receive 
		//osSemaphoreRelease(uart_SemHandle);
    osDelay(100);
  }
