...
/* Definitions for mes_Queue */
osMessageQueueId_t mes_QueueHandle;
const osMessageQueueAttr_t mes_Queue_attributes = {
  .name = "mes_Queue"
};
...
int main(void)
{
  ...
  /* Create the queue(s) */
  /* creation of mes_Queue */
  mes_QueueHandle = osMessageQueueNew (5, sizeof(uint16_t), &mes_Queue_attributes);
...
void orangeTask(void *argument)
{
  ...
  {
		orangeTaskProfiler++;
		osMessageQueuePut(mes_QueueHandle, &valueToSend, 0, 0);
    osDelay(100);
  }
  /* USER CODE END 5 */
}
...
void redTask(void *argument)
{
  ...
  {
		redTaskProfiler++;
		osMessageQueueGet(mes_QueueHandle, &read_data, NULL, osWaitForever);
		send_string("this is red  ");
